...
1. `POST /tenants/{tenant_id}/status` sets a tenant to `active` or
   `suspended`. It requires a new `tenant:suspend` permission held by
   `PLATFORM_ADMINISTRATOR` alone, and is a separate endpoint from
   `PUT /tenants/{id}` so that status is never changed as a side effect
   of editing a company profile.

   A new permission rather than reusing `tenant:update`: that one is
   held by `COMPANY_ADMINISTRATOR` for editing their own company profile
   and is checked **globally**, so reusing it would have let any company
   administrator suspend any tenant. The first implementation did
   exactly that, and the test asserting a customer cannot suspend
   themselves is what caught it.
...
- **Authorization**: status changes require `tenant:suspend`, held by
  `PLATFORM_ADMINISTRATOR` alone. A company administrator holds
  `tenant:update` — a different permission, for profile fields — and
  cannot reach this endpoint, so a customer can neither suspend nor
  un-suspend themselves, nor anyone else.
