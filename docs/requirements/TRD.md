...
**Project:** Arc Enterprise AI Platform **Project Type:** 7-day
placement-oriented engineering project **Domain:** IT Services /
Enterprise AI **Status:** Revised --- Final Consistency Review Draft
**Primary Technical Focus:** PII Guard, Company Brain, Secure RAG,
Unified Intelligence, Skills, AI Tools, Webhooks, Observability
**Deployment Strategy:** Local Docker-first → AWS deployment **Primary
Development Language:** Python 3.12 **AI Provider:** OpenRouter with
configurable model selection **AI Routing:** OmniRoute / configurable
routing where used
...
This Technical Requirements Document translates the Arc Product
Requirements Document into a practical technical implementation
direction.
...
This document is intentionally designed for a **7-day placement-oriented
engineering project**.

The goal is to demonstrate that the team understands and can implement
an end-to-end enterprise AI workflow.

The TRD does **not** require every conceptual component to become a
...
The following is the current proposed technical direction.
  ...
  ----------------------- ----------------------- -------------------------
  Language                Python 3.12             All developers are Python
                                                  developers and Python has
                                                  a strong AI, API, data,
                                                  and testing ecosystem.

  Backend API             FastAPI                 Lightweight Python API
                                                  framework suitable for
                                                  rapid development and
                                                  async/webhook workloads.

  Database                PostgreSQL              Provides reliable
                                                  structured persistence
                                                  for tenants, users,
                                                  skills, incidents, audit
                                                  records, and Company
                                                  Brain metadata.

  Vector Search           pgvector                Allows semantic retrieval
                                                  while keeping vector
                                                  storage close to
                                                  PostgreSQL instead of
                                                  introducing a separate
                                                  vector database.

  PII Protection          Microsoft Presidio      Open-source PII
                                                  detection/anonymization
                                                  tooling that integrates
                                                  naturally with Python and
                                                  can run locally or in
                                                  Docker.

  LLM Provider            OpenRouter              Provides configurable
                                                  access to multiple models
                                                  through a common API.

  LLM Model               Configurable            Prevents the project from
                                                  being tied to one model
                                                  and allows model
                                                  selection based on
                                                  quality, availability,
                                                  tool-calling support, and
                                                  cost.

  Embeddings              Configurable embedding  Keeps semantic retrieval
                          model                   independent of one fixed
                                                  embedding provider/model.

  AI Tools                Tool-agnostic           Allows suitable
                          interfaces              open-source
                                                  tools/frameworks to be
                                                  integrated or replaced
                                                  without changing the
                                                  product-level
                                                  architecture.

  Webhooks                FastAPI HTTP endpoints  Provides a simple and
                                                  locally testable
                                                  mechanism for receiving
                                                  external events.

  Containers              Docker                  Provides reproducible
                                                  development and execution
                                                  environments.

  Local orchestration     Docker Compose          Allows the application
                                                  and required local
                                                  dependencies to run
                                                  consistently.
  ...
  Formatting/Linting      Ruff                    Fast Python linting and
                                                  formatting with minimal
                                                  configuration.

  CI                      GitHub Actions          Provides automated
                                                  testing and validation
                                                  for pull requests.

  Cloud                   AWS                     Provides a practical
                                                  cloud deployment target
                                                  after local Docker
                                                  validation.
  -------------------------------------------------------------------------

These are **implementation proposals for the current project scope**.
Any technology that becomes architecture-significant should be confirmed
through the appropriate engineering review/ADR process.

------------------------------------------------------------------------

# 4. Architecture Principles

> The canonical architecture diagram is maintained in the PRD only. This
> TRD describes the technical responsibilities and constraints behind
> it.
...
The implementation should use the simplest structure that demonstrates
...
The distinction is therefore about responsibility, not separate
intelligence systems.

------------------------------------------------------------------------

# 5. Technical Architecture Interpretation

The PRD contains the canonical product-level architecture diagram. The
TRD intentionally does not duplicate that diagram.

Arc is implemented as a small application-level system containing
logical responsibilities for authentication/RBAC, tenant context, PII
protection, Company Brain, Secure RAG, Skills, Unified Intelligence, AI
Tools, Webhooks, and observability.

These are logical components, not mandatory microservices. The
implementation should use the smallest number of deployable components
that can clearly demonstrate the required behavior.

------------------------------------------------------------------------

# 6. Runtime Boundaries

The Arc application runtime is responsible for API handling,
authentication, tenant context, PII protection, Company Brain access,
Secure RAG, Skills, Unified Intelligence, AI Tools, Webhooks, and
observability.

The current external dependencies are PostgreSQL with pgvector,
OpenRouter with configurable models, optional OmniRoute routing, and
selected external connector APIs.

Only dependencies actually required by the implementation should be
provisioned. Conceptual components must not automatically become
separate containers.
...
Every tenant-scoped operation must establish tenant context before
...
-   tenant administration where explicitly authorized
...
-   manage authorized company knowledge, procedures, and Skills
-   review tenant incidents and usage
...
-   assigned tenant and explicitly permitted operational resources
...
These role definitions must remain consistent with the PRD.
Authentication establishes identity; RBAC and resource authorization
determine access.
...
Unified Intelligence is the combined Company Brain + Agent capability of
Arc.
...
The Agent must operate using Company Brain context for company-specific
workflows.
...
Documents and knowledge can be represented through embeddings to support
semantic retrieval.
...
Structured Company Brain data is stored in PostgreSQL; embeddings are
stored through pgvector.
...
The LLM provides language understanding, reasoning, planning, and
generation capabilities for Unified Intelligence.
...
The application establishes identity, tenant context, authorization, PII
handling, and permitted retrieval before sending context to the LLM. The
LLM returns a response or a structured request for an approved tool.
...
They allow semantically similar knowledge to be retrieved even when the
query does not use exactly the same words as the stored content.
...
Knowledge is chunked, embedded, and stored as vectors in pgvector.
Queries are embedded using the configured embedding model.

For a query:

The query is embedded, matched against stored vectors, and used to
retrieve candidate knowledge. Tenant and permission filtering occurs
before the content is supplied to the LLM.

------------------------------------------------------------------------

## 11.4 Embeddings vs LLM

Embeddings represent semantic meaning and enable retrieval. The LLM
understands language, reasons over retrieved context, and generates
responses or tool requests.
...
Secure retrieval first establishes tenant and permission context,
performs semantic retrieval, then filters candidate documents before
producing approved context.
...
AI Tools are controlled interfaces through which Unified Intelligence
can interact with Arc or approved external systems.
...
Tool execution requires selection, authorization, input validation,
execution, and result handling. Every controlled action is observable.
...
The LLM must not be allowed to execute arbitrary Python code or
...
Suitable open-source AI libraries/frameworks may be used where they
provide real value.
...
Arc will use **Microsoft Presidio** for the initial PII protection
layer.

Presidio is an open-source framework for detecting, analyzing, and
...
Incoming text is analyzed by Presidio, configured PII is anonymized, and
sanitized content is passed to downstream AI processing.

------------------------------------------------------------------------

## 15.4 PII Boundary

PII Guard should be applied before sensitive information crosses defined
AI/data boundaries.

The first implementation should focus on practical text PII protection.

Image PII redaction is not required merely because the technology can
support it.
...
Presidio uses automated detection and cannot guarantee detection of
...
Webhook events are validated, associated with a tenant, passed through
PII protection where required, and processed by Unified Intelligence.
...
Webhook payloads must not unnecessarily expose sensitive information in
logs.
...
Health monitoring should expose whether important Arc components are
functioning.

Examples:

Health monitoring covers application, database, AI provider, webhook,
RAG, and tool health.
...
Important failures should be observable and capable of creating or
contributing to an incident.

Example:

Important failures can degrade health status, create incidents, trigger
investigation, and lead to resolution or human action.
...
High-risk actions require human approval before the authorized tool is
executed. Rejection or failure stops or escalates the workflow.
...
The project will use a simple user identity model suitable for
approximately:
...
The exact authentication implementation must remain consistent with the
approved PRD and engineering decision.
...
Development should use `.env.example` for variable documentation and
...
The application uses documented environment variables for runtime
configuration; real secrets must remain outside Git.
...
Used when the project uses OmniRoute as the configurable AI routing
layer.
...
The local environment should allow a developer to start the project
...
Docker Compose should be used where multiple local dependencies are
actually required.

The project should not create unnecessary containers for conceptual
components.
...
CI should install dependencies, validate lint/formatting, run unit tests
and applicable integration tests, and build the Docker image.

CI should not require expensive cloud infrastructure merely to validate
the project.
...
After the local Docker workflow is stable and validated, Arc should be
deployable to AWS.

The AWS deployment should reuse the containerized application rather
than creating a completely different architecture.

The exact AWS service mapping remains a platform/implementation
decision.

The AWS target should not block local development.

The project does not require a complex multi-region or enterprise
...
Security is enforced from identity through authorization, tenant
context, PII handling, permission-aware retrieval, LLM use, and tool
authorization. The LLM cannot bypass these controls.
...
Local Docker-first execution uses Docker Compose for the Arc application
and only the required local dependencies.
...
After local validation, the Dockerized application should be deployable
to AWS. The exact AWS service mapping remains an implementation
decision.
...
Because this is a 7-day placement project, performance targets should
focus on measurable behavior rather than production-scale capacity.
...
Exact numerical targets may be established during implementation if
useful.
...
The implementation should nevertheless avoid unnecessary architectural
choices that make future scaling impossible.

The design should be understandable enough that a developer can explain
how:
...
The project may use approximately 2--3 useful real connectors where they
are:
...
Controlled/fake APIs are acceptable where real integrations introduce
unnecessary complexity or cost.

Connector selection remains an implementation planning decision and
should not dictate unnecessary platform infrastructure.
...
The project should support changing the model without changing the
...
The team may use suitable open-source libraries/frameworks where they
provide clear value.
...
The framework must not become more important than understanding the
underlying agent/tool architecture.
...
Architecture-significant decisions should be recorded through ADRs when
appropriate.
...
Recommended order: environment and API → persistence →
authentication/tenant/RBAC → PII → Company Brain → embeddings/Secure RAG
→ Skills → Unified Intelligence → AI Tools → Webhooks → observability →
automated actions/human escalation → tests → CI → local demonstration →
AWS follow-on.

This ordering is intended to support the 7-day implementation and
learning objective.
...
The minimum demonstration is an authenticated tenant-scoped request that
passes through PII protection, permission-aware Company Brain retrieval,
Unified Intelligence reasoning, Skill selection, an authorized AI Tool,
a controlled action, and observability, with incident or human
escalation on failure/high risk.
...
Tenant boundaries, authorization, PII controls, and tool permissions are
enforced.
...
Another developer can run the system locally using the documented Docker
workflow.
...
The completed system should demonstrate the following connected
architecture:

The final technical requirements are those listed in this TRD; the
product-level architecture diagram remains only in the PRD.

The project should remain a single understandable system rather than
becoming an unnecessarily distributed enterprise architecture.
...
-   The technical architecture remains proportional to the 7-day
    project.
...
The PRD remains the product source of truth for product behavior and
scope.

The TRD translates the approved product requirements into a practical
technical direction. The PRD contains the canonical product-level
architecture diagram; the TRD intentionally avoids duplicating it.

The PRD defines product behavior; the TRD defines technical direction;
ADRs record significant architecture decisions; implementation and tests
realize the approved design.

If a technical implementation requires changing a product requirement,
the PRD must be reviewed rather than silently changed through
implementation.
...
Bala and the engineering team review the technical direction and
identify any architecture decisions that require ADRs before
implementation.
